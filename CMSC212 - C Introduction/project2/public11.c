 ...
 * Tests running program using run() when max_instr is a positive number, one
 * which run() should eventually reach.
 */

int run_code(const char *filename);
...
int run_code(const char *filename) {
  ...
  if (num_instructions == 0)
    ...
    status= run(program, reg, freg, -1, 4 + (10 * 4));

    if (status != RUNNING)
      printf("\nError - run() returned non-RUNNING status %d.\n", status);
    else {
      printf("\n");
